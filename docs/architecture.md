...
**Game Room Servers** are the most architecturally significant component. Each room runs as a **logical cluster of three replicas**, but **leader election is delegated to etcd** rather than embedding a Raft library. On startup, all three replicas race to acquire an etcd lease on a key specific to the match ID. The winner becomes the leader and renews the lease every second. The leader processes all player inputs, advances the authoritative game state tick by tick, and writes committed state to Redis (hot cache) and to Kafka as an immutable event log. If the leader pod dies, its etcd lease expires; one of the two followers acquires the lease and becomes the new leader within seconds. State durability is guaranteed by Kafka's immutable event log, not by an in-memory Raft log. Every committed state change is published to Kafka as a movement event, which serves both the replay pipeline and the analytics pipeline downstream.
...
**Kubernetes** orchestrates all services. Game room pods scale horizontally via Horizontal Pod Autoscaler triggered by matchmaking demand. StatefulSets with stable DNS are used for game room replicas, as each logical game room needs a stable identity for etcd lease ownership and service discovery.

**etcd** stores distributed configuration and is the coordination backend for leader discovery across services. Critically, etcd's **lease and watch mechanism** replaces the need to embed a consensus library in the application layer — game room replicas use etcd to elect a leader, guaranteeing exactly-one active processor per match without custom consensus code. Kubernetes itself uses etcd internally, and the project uses it explicitly for service-level configuration and leader election, reinforcing understanding of why ZooKeeper was the prior art and what replaced it.
...
If the game room leader pod crashes mid-match, its etcd lease expires. The followers, which are actively watching the lease key, detect the expiration and race to re-acquire the lease. One follower wins and is promoted to leader within seconds. The new leader resumes by reading the latest committed state from Redis and continuing from the last Kafka offset. Players experience a brief freeze, then the game continues — no match loss, no data loss. When a player's WebSocket drops, the Reconnect Handler reads their last state delta from Redis and sends a compressed rejoin payload. The player re-enters without replaying the full event history.
