...
echo "--- Registration ---"
ALICE=$(_curl -X POST "$BASE_URL/auth/register" \
  -H "Content-Type: application/json" \
  -d '{"username":"alice","password":"pass123"}')
ALICE_TOKEN=$(echo "$ALICE" | _json_field access_token)

if [ -n "$ALICE_TOKEN" ]; then
  echo "  Alice registered: ${ALICE_TOKEN:0:20}..."
else
  echo "  Alice register response: $ALICE"
  ALICE=$(_curl -X POST "$BASE_URL/auth/login" \
    -H "Content-Type: application/json" \
    -d '{"username":"alice","password":"pass123"}')
  ALICE_TOKEN=$(echo "$ALICE" | _json_field access_token)
...
BOB=$(_curl -X POST "$BASE_URL/auth/register" \
  -H "Content-Type: application/json" \
  -d '{"username":"bob","password":"pass123"}')
BOB_TOKEN=$(echo "$BOB" | _json_field access_token)

if [ -n "$BOB_TOKEN" ]; then
  echo "  Bob registered: ${BOB_TOKEN:0:20}..."
else
  echo "  Bob register response: $BOB"
  BOB=$(_curl -X POST "$BASE_URL/auth/login" \
    -H "Content-Type: application/json" \
    -d '{"username":"bob","password":"pass123"}')
  BOB_TOKEN=$(echo "$BOB" | _json_field access_token)
...
ALICE_VALID=$(_curl "$BASE_URL/auth/validate" \
  -H "Authorization: Bearer $ALICE_TOKEN")
echo "  Alice valid: $(echo "$ALICE_VALID" | _json_field valid)"

BOB_VALID=$(_curl "$BASE_URL/auth/validate" \
  -H "Authorization: Bearer $BOB_TOKEN")
echo "  Bob valid: $(echo "$BOB_VALID" | _json_field valid)"
echo ""

# 4. Get player profiles
echo "--- Player Profiles ---"
ALICE_PROFILE=$(_curl "$BASE_URL/auth/validate" \
  -H "Authorization: Bearer $ALICE_TOKEN")
ALICE_PID=$(echo "$ALICE_PROFILE" | _json_field player_id)

if [ -n "$ALICE_PID" ]; then
  PROFILE=$(_curl "$BASE_URL/players/$ALICE_PID" \
    -H "Authorization: Bearer $ALICE_TOKEN")
  echo "  Alice: $PROFILE"
fi
echo ""
...
# 5. Leaderboard
echo "--- Leaderboard ---"
LB=$(_curl "$BASE_URL/leaderboard")
echo "  $LB" | python3 -m json.tool 2>/dev/null || echo "  $LB"
echo ""

# 6. Matchmaking
echo "--- Matchmaking Queue ---"
MM_RESULT=$(_curl -X POST "$BASE_URL/matchmaking/queue" \
  -H "Content-Type: application/json" \
  -d "{\"token\":\"$ALICE_TOKEN\"}")
...
RECONNECT=$(_curl -X POST "$BASE_URL/reconnect" \
  -H "Content-Type: application/json" \
  -d "{\"token\":\"$ALICE_TOKEN\",\"match_id\":\"test-match-001\"}")
...
REPLAY=$(_curl "$BASE_URL/replay/test-match-001" || echo "{}")
...
_curl "$BASE_URL/metrics" | grep -E "^(auth_|matchmaking_|gameroom_|analytics_|leaderboard_)" | head -20 || echo "  No custom metrics found"
echo ""

# 10. Grafana
echo "--- Grafana ---"
GF_STATUS=$(_curl -o /dev/null -w "%{http_code}" http://localhost:3001 || echo "unreachable")
echo "  Grafana UI: http://localhost:3001 (status: $GF_STATUS)"
echo ""

# 11. Game Client
echo "--- Game Client ---"
GC_STATUS=$(_curl -o /dev/null -w "%{http_code}" "$GAME_URL" || echo "unreachable")
echo "  Game Client: $GAME_URL (status: $GC_STATUS)"
echo ""

# 12. Jaeger
echo "--- Jaeger ---"
J_STATUS=$(_curl -o /dev/null -w "%{http_code}" http://localhost:16686 || echo "unreachable")
